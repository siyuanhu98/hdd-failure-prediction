...
import warnings
import argparse
from utils.loadDataTH import read_data1 as read_data
...
def LightGBM_regressor(train_x, train_y):
    ...
    model_LightGBMRegressor = lgb.sklearn.LGBMRegressor(n_estimators=100,max_depth=2)
...
def LightGBM_classifier(train_x, train_y):
    from sklearn import ensemble
    model_LightGBMClassifier = lgb.sklearn.LGBMClassifier(n_estimators=100,max_depth=2)
...
def model_fit(train_X, train_y, test_X, test_y):
    ...
    for regressor in test_regressor:
        ...
        pickle.dump(model, open('../model-2/TH/LightGBM_model_%s.h5' % t0, 'wb'))
